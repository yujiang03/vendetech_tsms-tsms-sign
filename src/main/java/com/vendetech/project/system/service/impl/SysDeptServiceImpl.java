...
import com.vendetech.project.system.mapper.SysDeptMapper;
...
public class SysDeptServiceImpl implements ISysDeptService
{
	...
	public List<Department> getDepartmentList(SysUser currentUser) {
		...
		return getChildPerms(list, "1");
	}

	/**
	 * @param ddDepartmentId
	 * @return
	 */
	@Override
	public List<Department> getDepartmentList(String ddDepartmentId) {
		List<Department> list = deptMapper.getDepartmentList();
		return getChildPerms(list, ddDepartmentId);
	}
	...
	public List<DeptListVo> buildDepts(List<Department> depts) {
		List<DeptListVo> routers = new LinkedList<DeptListVo>();
		if (null == depts) {
			depts = new ArrayList<Department>();
		}
		for (Department dept : depts) {
			DeptListVo router = new DeptListVo();
			router.setTitle(dept.getDepartmentName());
			router.setValue(dept.getDepartmentId().toString());
			List<Department> cDepts = dept.getChildren();
			if (!cDepts.isEmpty() && cDepts.size() > 0 && "1".equals(dept.getParentDepartmentId())) {
				router.setChildren(buildCDepts(cDepts));
			}
			routers.add(router);
		}
		return routers;
    ...
    public List<DeptListVo> buildCDepts(List<Department> depts){
    	List<DeptListVo> routers = new LinkedList<DeptListVo>();
    	for (Department dept : depts)
        {
    		DeptListVo router = new DeptListVo();
            ...
            router.setValue(dept.getDepartmentId().toString());
            List<Department> cDepts = dept.getChildren();
            if (!cDepts.isEmpty() && cDepts.size() > 0)
            {
                router.setChildren(buildCDepts(cDepts));
            }
            routers.add(router);
	 ...
	 * @return String
	 */
	public List<Department> getChildPerms(List<Department> list, String parentId) {
		List<Department> returnList = new ArrayList<Department>();
		for (Iterator<Department> iterator = list.iterator(); iterator.hasNext();) {
			Department t = (Department) iterator.next();
			// 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
			if ("1".equals(parentId)) {
				if ("1".equals(t.getParentDepartmentId())) {
					recursionFn(list, t);
					returnList.add(t);
				}
			} else {
				if (parentId.equals(t.getDdDepartmentId())) {
					recursionFn(list, t);
					returnList.add(t);
				}
			}
	...
	private List<Department> getChildList(List<Department> list, Department t) {
		...
		while (it.hasNext()) {
			Department n = (Department) it.next();
			if (n.getParentDepartmentId().equals(t.getDdDepartmentId())) {
				tlist.add(n);
			}
		}
		return tlist;
